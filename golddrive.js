document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!window.location.search.includes('redirected=true') || document.innerWidth <= 1262) {
        if (isMobile) {
...
//     const square_languages_opacity_background = document.querySelector('.square_languages.opacity_background'); // Замените на ваш селектор
//     if (window.innerHeight >= 660) {
//         const currentWidth = parseInt(getComputedStyle(square_languages_opacity_background).width); // Получаем текущую ширину в числовом формате
//         square_languages_opacity_background.style.width = `${currentWidth + 20}px`; // Увеличиваем ширину на 1px
//     } else {
//         square_languages_opacity_background.style.width = ''; // Сбрасываем стиль ширины, если нужно
//     }
// });
...
armenian.addEventListener('click', function(event) {
    ...
    english.style.transition = "all 0.3s ease"
    russion.style.transition = "all 0.3s ease"
})
